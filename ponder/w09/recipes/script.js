...
document.addEventListener('DOMContentLoaded', () => {
	...
	const groupImageContainer = document.querySelector('.group-image');
	...
	function searchRecipes(keywords) {
		...
		return recipes.filter(recipe => {
			const nameMatch = recipe.name.toLowerCase();
			const descMatch = recipe.description.toLowerCase();
			const tagsMatch = recipe.tags.join(' ').toLowerCase();
	...
	function displayRecipe(recipe) {
		const firstTag = recipe.tags && recipe.tags.length > 0 ? recipe.tags[0] : '';
		
		groupImageContainer.style.flexDirection = '';
		groupImageContainer.innerHTML = `
			<img src="${recipe.image}" alt="${recipe.name}">
			<div class="subgroup-image">
				${firstTag ? `<button>${firstTag}</button>` : ''}
				<h1 class="food-title">${recipe.name.toUpperCase()}</h1>
	...
	function displayRecipes(recipeList) {
		if (recipeList.length === 0) {
			groupImageContainer.innerHTML = '<p>No recipes found. Try a different search.</p>';
			return;
		}
		...
		groupImageContainer.innerHTML = '';
		groupImageContainer.style.flexDirection = 'column';
		
		recipeList.forEach(recipe => {
			const firstTag = recipe.tags && recipe.tags.length > 0 ? recipe.tags[0] : '';
			
			const recipeDiv = document.createElement('div');
			recipeDiv.className = 'group-image';
			...
			`;
			groupImageContainer.appendChild(recipeDiv);
		});
	}
	...
	const searchIcon = document.querySelector('.input-search img');
	if (searchIcon) {
